...
// browser (localStorage) so they survive a reload. Loading one restores its
// inputs and re-runs the analysis; editing inputs afterwards marks it "edited"
// so you always know whether you're looking at the saved version.
let scenarios = [];            // [{ id, name, params }]
let activeScenarioId = null;   // scenario whose inputs are currently loaded
let scenarioDirty = false;     // inputs changed since that scenario was loaded/saved
let lastResult = null;
const SCENARIO_STORE_KEY = 'hde-scenarios-v1';
const MAX_SCENARIOS = 6;

function snapshotScenario() { return lastResult ? Object.assign({}, lastResult) : null; }
...
// ---- Pure result computation (no DOM) — used for the comparison table so saved
// scenarios are always evaluated by the CURRENT engine, never a stale snapshot.
function computeResultForParams(params) {
  const p = params;
  const own = calcTotalOwnership(p.homePrice, p.downPct, p.rate, p.term, p.taxPct, p.insuranceAnnual, p.maintPct, p.hoaMonthly, p.creditProfile, p.loanType);
  const net = own - calcEffectiveRental(p.rentalIncome, p.vacancyPct, p.expenseRatioPct);
...
// ---- Persistence (best-effort; private mode / disabled storage is fine) ----
function persistScenarios() {
  try { localStorage.setItem(SCENARIO_STORE_KEY, JSON.stringify(scenarios.map(s => ({ id: s.id, name: s.name, params: s.params })))); } catch (_) {}
}
function restoreScenarios() {
  try {
    ...
    if (Array.isArray(arr)) scenarios = arr.filter(s => s && s.params && typeof s.params.homePrice === 'number').slice(0, MAX_SCENARIOS);
  } catch (_) { scenarios = []; }
}

// ---- Form fingerprint: detects edits since a scenario was loaded ----
function readFormFingerprint() {
  const parts = SCENARIO_INPUT_MAP.map(([inputId]) => { const el = $(inputId); return el ? String(el.value) : ''; });
  parts.push($('moveOutYear') ? String($('moveOutYear').value) : '');
  return parts.join('|');
}
let activeFingerprint = '';
function refreshScenarioDirty() {
  if (!activeScenarioId) { scenarioDirty = false; return; }
  const wasDirty = scenarioDirty;
  scenarioDirty = readFormFingerprint() !== activeFingerprint;
  if (wasDirty !== scenarioDirty) updateScenarioUI();
}

function uniqueScenarioName(base) {
  let name = base, n = 2;
  while (scenarios.some(s => s.name.toLowerCase() === name.toLowerCase())) name = `${base} (${n++})`;
  return name;
}

// ---- Actions ----
function saveCurrentScenario() {
  if (scenarios.length >= MAX_SCENARIOS) { flashScenarioNote(`You can keep up to ${MAX_SCENARIOS} scenarios — delete one to save another.`); return; }
  if (!calculate()) { flashScenarioNote('Fix the highlighted input first — nothing was saved.'); return; }
  const input = $('scenarioNameInput');
  const typed = input ? input.value.trim() : '';
  const name = uniqueScenarioName(typed || `Scenario ${scenarios.length + 1}`);
  const id = 's' + Date.now().toString(36) + Math.random().toString(36).slice(2, 6);
  scenarios.push({ id, name, params: structuredClone(lastResult.params) });
...
function loadScenario(id) {
  ...
  if (!s) return;
  loadScenarioIntoInputs({ params: s.params });
  activeScenarioId = id; activeFingerprint = readFormFingerprint(); scenarioDirty = false;
  calculate();
  updateScenarioUI();
}
...
function updateActiveScenario() {
  ...
  if (!s) return;
  if (!calculate()) { flashScenarioNote('Fix the highlighted input first — nothing was updated.'); return; }
  s.params = structuredClone(lastResult.params);
  activeFingerprint = readFormFingerprint(); scenarioDirty = false;
  persistScenarios(); updateScenarioUI();
  flashScenarioNote(`Updated “${s.name}” with your current inputs.`);
}

function revertActiveScenario() { if (activeScenarioId) loadScenario(activeScenarioId); }

function deleteScenario(id) {
  const idx = scenarios.findIndex(x => x.id === id);
  if (idx < 0) return;
  const [gone] = scenarios.splice(idx, 1);
  if (activeScenarioId === id) { activeScenarioId = null; scenarioDirty = false; }
  persistScenarios(); updateScenarioUI();
  flashScenarioNote(`Deleted “${gone.name}”.`);
}
...
function renameScenario(id, newName) {
  ...
  if (!clean || clean === s.name) { updateScenarioUI(); return; }
  s.name = scenarios.some(o => o.id !== id && o.name.toLowerCase() === clean.toLowerCase()) ? uniqueScenarioName(clean) : clean;
  persistScenarios(); updateScenarioUI();
}

function clearScenarios() {
  scenarios = []; activeScenarioId = null; scenarioDirty = false;
  persistScenarios(); updateScenarioUI();
}

let scenarioNoteTimer = null;
function flashScenarioNote(text) {
  const el = $('scenarioNote');
  if (!el) return;
  el.textContent = text; el.hidden = false;
  clearTimeout(scenarioNoteTimer);
  scenarioNoteTimer = setTimeout(() => { el.hidden = true; }, 2600);
}

// ---- Rendering ----
function escapeHtml(s) { return String(s).replace(/[&<>"']/g, c => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' }[c])); }

function scenarioMeta(p) {
  const bits = [fmt(p.homePrice), `${p.downPct}% down`, `${p.rate}%`, `${p.timeHorizon}yr`];
  if (p.rentalIncome > 0) bits.push('house hack');
  return bits.join(' · ');
}
...
function updateScenarioUI() {
  ...
  // Count + save button state
  if ($('scenarioCount')) $('scenarioCount').textContent = scenarios.length ? `${scenarios.length} of ${MAX_SCENARIOS}` : '';
  if ($('btnScenarioSave')) $('btnScenarioSave').disabled = scenarios.length >= MAX_SCENARIOS;

  // "Edited since load" row
  const dirtyRow = $('scenarioDirtyRow');
  if (dirtyRow) {
    if (active && scenarioDirty) {
      dirtyRow.style.display = '';
      $('scenarioDirtyText').innerHTML = `Inputs changed since you loaded <b>${escapeHtml(active.name)}</b>. Save as new, or:`;
    } else {
  ...
  list.innerHTML = scenarios.map(s => {
    ...
    const tag = isActive ? (scenarioDirty ? '<span class="scen-tag scen-tag-edited">edited</span>' : '<span class="scen-tag scen-tag-active">viewing</span>') : '';
    return `<li class="scen-item${isActive ? ' scen-item-active' : ''}" data-id="${s.id}">
      <button type="button" class="scen-load" data-act="load" data-id="${s.id}" title="Load this scenario">
        <span class="scen-name">${escapeHtml(s.name)}</span>${tag}
        <span class="scen-meta">${escapeHtml(scenarioMeta(s.params))}</span>
      </button>
      <span class="scen-actions">
        <button type="button" class="scen-icon" data-act="rename" data-id="${s.id}" title="Rename" aria-label="Rename ${escapeHtml(s.name)}">✎</button>
        <button type="button" class="scen-icon scen-icon-del" data-act="delete" data-id="${s.id}" title="Delete" aria-label="Delete ${escapeHtml(s.name)}">×</button>
      </span>
    </li>`;
  }).join('');
  if ($('scenarioEmpty')) $('scenarioEmpty').style.display = scenarios.length ? 'none' : '';

  // Badge in the results column
  const badge = $('scenarioActiveBadge');
  if (badge) {
    if (active) { badge.textContent = `Viewing: ${active.name}${scenarioDirty ? ' (edited)' : ''}`; badge.style.display = ''; }
  ...
  if (panel) {
    if (scenarios.length >= 2) { panel.style.display = ''; renderComparison(); }
    else panel.style.display = 'none';
  }
}

function renderComparison() {
  const results = scenarios.map(s => Object.assign({ id: s.id, name: s.name }, computeResultForParams(s.params)));
  const best = results.reduce((a, b) => (b.wealth5.wealthImpact > a.wealth5.wealthImpact ? b : a), results[0]);
  const rows = [
    { label: 'Home price', get: r => fmt(r.params.homePrice) },
    { label: 'Down payment', get: r => `${r.params.downPct}% (${fmt(r.params.homePrice * r.params.downPct / 100)})` },
    { label: 'Rate / term', get: r => `${r.params.rate}% / ${r.params.term}yr` },
    { label: 'Rent compared', get: r => fmt(r.params.rent) + '/mo' },
    { label: 'Horizon', get: r => `${r.params.timeHorizon} years` },
    { label: 'Monthly cost to own', get: r => fmt(r.params.rentalIncome > 0 ? r.netCost : r.totalOwnership) + (r.params.rentalIncome > 0 ? ' (after rent)' : '') },
    { label: 'vs rent', get: r => fmtSigned(r.monthlyDiff) + '/mo', cls: r => r.monthlyDiff > 0 ? 'neg' : 'pos' },
    { label: 'Wealth impact at horizon', get: r => fmtSigned(r.wealth5.wealthImpact), cls: r => r.wealth5.wealthImpact >= 0 ? 'pos' : 'neg', strong: true },
    { label: 'Break-even', get: r => r.breakEven.found ? fmtBreakEven(r.breakEven) : `Not within ${Math.round(r.breakEven.scanYears)} yrs` },
    { label: 'Recommendation', get: r => ({ buy: 'Buy', hack: 'Buy (house hack)', 'lean-rent': 'Lean rent', rent: 'Rent' }[r.decision.type] || r.decision.type) + ` · ${r.confidence.label}` },
  ];
  const thead = document.querySelector('#comparePanel thead tr');
  if (thead) thead.innerHTML = '<th scope="col">Metric</th>' + results.map(r =>
    `<th scope="col" class="${r.id === activeScenarioId ? 'cmp-active' : ''}">${escapeHtml(r.name)}${r.id === best.id && results.length > 1 ? ' <span class="cmp-best">best</span>' : ''}</th>`).join('');
  $('compareBody').innerHTML = rows.map(row =>
    `<tr><td class="cmp-label" scope="row">${row.label}</td>` + results.map(r =>
      `<td class="${row.cls ? row.cls(r) : ''}${row.strong ? ' cmp-strong' : ''}">${row.get(r)}</td>`).join('') + '</tr>').join('');
  const others = results.filter(r => r.id !== best.id);
  const runnerUp = others.length ? others.reduce((a, b) => (b.wealth5.wealthImpact > a.wealth5.wealthImpact ? b : a), others[0]) : null;
  const sameHorizon = results.every(r => r.params.timeHorizon === results[0].params.timeHorizon);
  $('compareSummary').textContent = runnerUp
    ? `Best on total wealth: ${best.name} — ${fmt(Math.abs(best.wealth5.wealthImpact - runnerUp.wealth5.wealthImpact))} ahead of ${runnerUp.name}${sameHorizon ? '' : ' (note: different time horizons)'}.`
    : '';
}

// ---- Inline rename ----
function startRenameScenario(id) {
  const item = document.querySelector(`.scen-item[data-id="${id}"]`);
  const s = scenarios.find(x => x.id === id);
  if (!item || !s) return;
  const inp = document.createElement('input');
  inp.type = 'text'; inp.value = s.name; inp.maxLength = 40; inp.className = 'scen-label-edit';
  inp.setAttribute('aria-label', 'Scenario name');
  item.innerHTML = ''; item.appendChild(inp); inp.focus(); inp.select();
  let done = false;
  const commit = () => { if (done) return; done = true; renameScenario(id, inp.value); };
  inp.addEventListener('blur', commit);
  inp.addEventListener('keydown', e => { if (e.key === 'Enter') inp.blur(); if (e.key === 'Escape') { done = true; updateScenarioUI(); } });
}
...
(function initScenarioUI() {
  ...
  list.addEventListener('click', e => {
    ...
    if (act === 'load') loadScenario(id);
    else if (act === 'rename') startRenameScenario(id);
    else if (act === 'delete') {
      if (btn.classList.contains('scen-confirm')) { deleteScenario(id); return; }
      btn.classList.add('scen-confirm'); btn.textContent = 'Delete?'; btn.title = 'Click again to confirm';
      setTimeout(() => { if (btn.isConnected) { btn.classList.remove('scen-confirm'); btn.textContent = '×'; btn.title = 'Delete'; } }, 2500);
    }
  });
  const nameInput = $('scenarioNameInput');
  if (nameInput) nameInput.addEventListener('keydown', e => { if (e.key === 'Enter') { e.preventDefault(); saveCurrentScenario(); } });
  // Any edit to the inputs column marks the loaded scenario as edited
  const inputsPanel = document.querySelector('.inputs-panel');
  if (inputsPanel) {
    inputsPanel.addEventListener('input', e => { if (e.target && e.target.id !== 'scenarioNameInput') refreshScenarioDirty(); });
    inputsPanel.addEventListener('change', e => { if (e.target && e.target.id !== 'scenarioNameInput') refreshScenarioDirty(); });
  }
  restoreScenarios();
  updateScenarioUI();
})();
...
function resetForm() {
  ...
  if (resultsVisible) {
    const ok = window.confirm(
      'Reset all inputs to defaults?\n\nYour current analysis and unsaved changes will be cleared. Saved scenarios are kept.'
    );
    if (!ok) return;
  }
  ...
  activeScenarioId = null; scenarioDirty = false;
